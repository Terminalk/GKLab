    ...
    private void draw(GL2 gl2) {
        ...
        switch(objectNumber) {
            case 1:
                ...
                int wielkosc = 3;
                for (int i = 1; i <= w; i++) {
                    gl2.glColor3f( 1.0f, i, i );
                    gl2.glBegin( GL2.GL_TRIANGLES );
                    gl2.glVertex3f( 0.0f, 3.0f, 0.0f );
                    for (int a = 0; a <= 1; a++) {
                        int s = i + a;
                        if(i >= w) s = 1;
                        double rad = ((kat * s) * (2 * Math.PI)) / 360;
                        float y = (float)Math.sin(rad) * wielkosc;
                        float x = (float)Math.cos(rad) * wielkosc;
                        gl2.glVertex3f( x, -1.0f, y );
                    }
                    gl2.glEnd();

                }

                // rysuj ostatni trojkat
                gl2.glColor3f( 1.0f, w, w );
                gl2.glBegin( GL2.GL_TRIANGLES );
                gl2.glVertex3f( 0.0f, 3.0f, 0.0f );
                double rad_l = ((kat * 1) * (2 * Math.PI)) / 360;
                float y_l = (float)Math.sin(rad_l) * wielkosc;
                float x_l = (float)Math.cos(rad_l) * wielkosc;
                gl2.glVertex3f( x_l, -1.0f, y_l );

                double rad_r = ((kat * w) * (2 * Math.PI)) / 360;
                float y_r = (float)Math.sin(rad_r) * wielkosc;
                float x_r = (float)Math.cos(rad_r) * wielkosc;
                gl2.glVertex3f( x_r, -1.0f, y_r );

                gl2.glEnd();

                gl2.glBegin(GL2.GL_TRIANGLE_FAN);
                gl2.glColor3f( 0.5f, 0.0f, 1.0f );
                for (int i = 1; i <= w; i++) {
                    double rad = ((kat * i) * (2 * Math.PI)) / 360;
                    float y = (float)Math.sin(rad) * wielkosc;
                    float x = (float)Math.cos(rad) * wielkosc;
                ...
                case 2:
                    ...
                    gl2.glColor3f( 1.0f, 1.0f, 0.0f ); // Red
                    float x = -0.01f;
                    float n = 5.0f;
                    for(double angle = 0.0f; angle <= (2.0f*3.14)*n; angle += 0.1f)
                    {
